...
class FolderTechRepository implements IFolderTechRepository
{
    ...
    public function save($request)
    {
        try {
            ...
            $folderTech->DATE_ENTRY = Carbon::parse($request->input('DATE_ENTRY'));
            $folderTech->DATE_LAI = Carbon::parse($request->input('DATE_LAI'));
    ...
    {
        try {
            return FolderTech::where("organisation_id", "=", $this->organisation_id)
                ->paginate($request['limit'], ['*'], 'page', $request['page']);
        } catch (\Exception $exception) {
            $this->Log($exception);
            return null;
    ...
    {
        try {
            $model= FolderTech::where("organisation_id",'=',$this->organisation_id)->find($id);
            $deleted = $model;
            $deleted->delete();
            return $model;
        } catch (\Exception $exception) {
