...
const FriendsList = ({ classes, friends, onSelect }) => {
    ...
    const filterFriends = (event, newVal) => {
        setFilteredFriends(newVal ? friends.filter(fr => fr.name && fr.name.includes(newVal)) : friends)
    }
            ...
            <Dialog open={open} onClose={() => setOpen(false)} aria-labelledby="form-dialog-title-update">
                <DialogTitle className={classes.title} className={classes.title} id="form-dialog-title">
                    <Autocomplete
                        ...
                        options={friends.map(fr => fr.name)}
                        renderInput={(params) => (
                            <TextField {...params} label="Nom" margin="normal" variant="outlined" />
                        )}
                    />
                </DialogTitle>
                        ...
                        && (
                            filteredFriends.map((friend, index) => (
                                <div key={friend.id + "-" + index} className={classes.card}>
                                    <FriendCard name={friend.name} onClick={() => handleClick(friend)} />
                                </div>
                            )
                            ))
                    }
