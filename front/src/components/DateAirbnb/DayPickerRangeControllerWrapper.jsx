...
class DayPickerRangeControllerWrapper extends React.Component {
    ...
    onDatesChange({ startDate, endDate }) {
        ...
        let  date
        if(endDate===null){
            date ={first:dayStart+' '+monthStart,second:'Départ',firstIso:moment(startDate).toISOString(),secondIso:moment(endDate).toISOString()}
            this.props.dispatch({
                type: "Add_Date",
                date:date
            })
        }else {
            date ={first:dayStart+' '+monthStart,second:dayEnd+' '+monthEnd,firstIso:moment(startDate).toISOString(),secondIso:moment(endDate).toISOString()}
            this.props.dispatch({
                type: "Add_Date",
                date:date
            })
    ...
    render() {
        const { showInputs,date } = this.props;
            ...
            <div style={{ height: '100%' }}>
                ...
                <DayPickerRangeController
                    ...
                    startDate={date.date !== undefined ?
                        date.date.firstIso==="" ? startDate:moment(date.date.firstIso):startDate}
                    endDate={date.date !== undefined ?
                        date.date.secondIso==="" ? endDate:moment(date.date.secondIso):endDate}
